...
function HttpAPI(discovery, settings) {
  ...
  function invokeWebhook(type, data) {
    ...
    // in case there's a single webhook in settings.webhook
    if (settings.webhook) {
      sendPOSTRequest(settings.webhook, jsonBody);
    }

    // in case we have multiple webhooks in settings.webhooks (an array)
    if (Array.isArray(settings.webhooks)) {
      settings.webhooks.forEach(hookUrl => {
        sendPOSTRequest(hookUrl, jsonBody);
      });
    }
  }

  function sendPOSTRequest(uri, jsonBody) {
    ...
    .catch(function (err) {
      logger.error('Could not reach webhook endpoint', uri, 'for some reason. Verify that the receiving end is up and running.');
      logger.error(err);
    });
  }
}
